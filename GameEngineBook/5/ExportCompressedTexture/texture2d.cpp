...
Texture2D* Texture2D::LoadFromFile(std::string& image_file_path) {
	...
	if (data) {
		...
		case 3:
		{
			...
			texture2d->gl_texture_format = GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
			break;
		}
		case 4:
		{
			image_data_format = GL_RGBA;
			texture2d->gl_texture_format = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
			break;
		}
		}
	}
	...
	glTexImage2D(GL_TEXTURE_2D, texture2d->mipmap_level, texture2d->gl_texture_format, texture2d->width, texture2d->height, 0, image_data_format, GL_UNSIGNED_BYTE, data);//将图像RGB数据上传至GPU
...
Texture2D* Texture2D::LoadFromCptFile(std::string& image_file_path) {
	...
	texture2d->gl_texture_format = cpt_file_header.gl_texture_format;
	texture2d->width = cpt_file_header.width;
	texture2d->height = cpt_file_header.height;

	glGenTextures(1, &(texture2d->gl_texture_id));//通知显卡创建纹理对象，返回句柄

	glBindTexture(GL_TEXTURE_2D, texture2d->gl_texture_id);//将纹理绑定到特定纹理目标

	glCompressedTexImage2D(GL_TEXTURE_2D, 0, texture2d->gl_texture_format, texture2d->width, texture2d->height, 0, cpt_file_header.compress_size, data);//将压缩纹理数据上传至GPU
...
void Texture2D::CompressImageFile(std::string& image_file_path, std::string& save_image_file_path) {
	...
	cpt_file_header.mipmap_level = texture2d->mipmap_level;
	cpt_file_header.width = texture2d->width;
	cpt_file_header.height = texture2d->height;
