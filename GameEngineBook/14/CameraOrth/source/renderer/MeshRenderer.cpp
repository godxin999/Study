...
void MeshRenderer::Render() {
	...
	glUseProgram(gl_program_id);
	{
		...
		for (auto&& [index, texture] : std::views::enumerate(textures)) {
			auto& [name, ptexture] = texture;
			GLint u_texture_location = glGetUniformLocation(gl_program_id, name.c_str());
			glActiveTexture(GL_TEXTURE0 + index);//激活纹理单元
			glBindTexture(GL_TEXTURE_2D, ptexture->gl_texture_id());//将加载的纹理句柄绑定到纹理单元
