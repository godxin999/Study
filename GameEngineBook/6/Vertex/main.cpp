...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUseProgram(program);
		{
			...
			glVertexAttribPointer(vpos_location, 3, GL_FLOAT, false, sizeof(Vertex), kVertices);

			//启用顶点着色器属性(a_color)，指定与顶点颜色数据进行关联
			glEnableVertexAttribArray(vcol_location);
			glVertexAttribPointer(vcol_location, 4, GL_FLOAT, false, sizeof(Vertex), ((float*)kVertices) + 3);

			glEnableVertexAttribArray(a_uv_location);
			glVertexAttribPointer(a_uv_location, 2, GL_FLOAT, false, sizeof(Vertex), ((float*)kVertices) + 3 + 4);
